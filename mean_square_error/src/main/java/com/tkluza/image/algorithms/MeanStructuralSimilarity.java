...
public class MeanStructuralSimilarity extends ImageSSIMAlgorithm {
	...
	String[] window_type = { "Gaussian", "Same weight" }; // WE CAN WEIGHTS THE WINDOW WITH A GAUSSIAN WEIGHTING
															// FUNCTION OR GIVING THE SAME WEIGHT TO ALL THE PIXELS IN
															// THE WINDOW
	String window_selection = window_type[0];
	String[] kind_of_algorithm = { "Zhou Wang", "Rouse/Hemami" }; // WE CAN USE THE INDEX FOR THE SUPRA-THRESHOLD LEVEL
	...
	int pointer, filter_length, image_height, image1_width, image_dimension, bits_per_pixel_1, bits_per_pixel_2, a, b,
			c;
	...
	public void prepareImages() {
		ImageConverter converter = new ImageConverter(image1);
		ImageConverter converter2 = new ImageConverter(image2);
	...
	protected void showDialog() {
		...
		while (!out && mode != Mode.DEFAULT) {
			out = true;
			gd = new GenericDialog("MS-SSIM Index calculation");
			...
			gd.addChoice("Show SSIM map (with exponents alfa, beta, gamma eq 1) at level: ", ssim_map_level_option,
					ssim_map_selection);
	...
	protected boolean validation() {
		...
		image_height = image1.getHeight();
		a = image2.getHeight();
		if (a != image_height) {
			IJ.error("Both images must have the same height");
			validationState = false;
		}
		image1_width = image1.getWidth();
		a = image2.getWidth();
		if (a != image1_width) {
			IJ.error("Both images must have the same width");
			validationState = false;
		}
		if ((image_height < 32) | (image1_width < 32)) {
			IJ.error("Miminum height and width must be 32 pixels");
			validationState = false;
		}
		bits_per_pixel_1 = image1.getBitDepth();
		bits_per_pixel_2 = image2.getBitDepth();
		...
		if ((image_height / downsampled < 32) | (image1_width / downsampled < 32)) {
	...
	public void actualAlgorithm() {
		...
		// MAIN ALGORITHM
		//
		ImageProcessor image_1_original_p = image1.getProcessor();
		ImageProcessor image_2_original_p = image2.getProcessor();
		image1_width = image_1_original_p.getWidth();
		image1_width = (int) (image1_width / downsampled); // YOU
															// CAN
															// DOWNSAMPLE
															// THE
															// IMAGE
		...
		image_1_p = image_1_original_p.resize(image1_width);
		image_2_p = image_2_original_p.resize(image1_width);
		...
		for (level = 1; level <= number_of_levels; level++) { // THIS LOOP CALCULATES, FOR EACH ITERATION, THE VALUES OF
																// L, C AND S
			...
			if (level != 1) {
				...
				image_1_p = image_1_p.resize(image1_width / 2);
				image_2_p = image_2_p.resize(image1_width / 2);
			}
			image_height = image_1_p.getHeight();
			image1_width = image_1_p.getWidth();
			image_dimension = image1_width * image_height;
			...
			ImageProcessor mu1_ip = new FloatProcessor(image1_width, image_height);
			ImageProcessor mu2_ip = new FloatProcessor(image1_width, image_height);
			...
			// PROCESSORS
			//
			ImageProcessor soporte_1_ip = new FloatProcessor(image1_width, image_height);
			ImageProcessor soporte_2_ip = new FloatProcessor(image1_width, image_height);
			ImageProcessor soporte_3_ip = new FloatProcessor(image1_width, image_height);
			...
			if (algorithm_selection == "Zhou Wang") {
				...
				for (pointer = 0; pointer < image_dimension; pointer++) {
					...
					contrast_point = (double) ((2 * sigma1[pointer] * sigma2[pointer] + C2)
							/ (sigma1_sq[pointer] + sigma2_sq[pointer] + C2));
					contrast[level] = contrast[level] + contrast_point;

					structure_point = (double) ((sigma12[pointer] + C2 / 2)
							/ (sigma1[pointer] * sigma2[pointer] + C2 / 2));
				...
				for (pointer = 0; pointer < image_dimension; pointer++) {
					...
					if ((sigma1_sq[pointer] + sigma2_sq[pointer]) == 0)
						...
						contrast_point = (double) ((2 * sigma1[pointer] * sigma2[pointer])
								/ (sigma1_sq[pointer] + sigma2_sq[pointer]));

					contrast[level] = contrast[level] + contrast_point;

					if (((sigma1[pointer] == 0) | (sigma2[pointer] == 0)) & (sigma1[pointer] != sigma2[pointer]))
						structure_point = 0;
					else if ((sigma1[pointer] == 0) & (sigma2[pointer] == 0))
						structure_point = 1;
					else
