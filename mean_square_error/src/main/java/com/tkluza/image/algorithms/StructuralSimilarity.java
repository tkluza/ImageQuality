...
/**
 * 
 * The equivalent of Zhou Wang's SSIM MatLab code as a Java plugin inside
 * ImageJ. from http://www.cns.nyu.edu/~zwang/files/research/ssim/index.html
 * November 27th 2008.
 * 
 ...
 * 26/11/2008.gprietor@med.ucm.es
 * 
 ...
 * 
 * Gabriel Prieto, Margarita Chevalier, Eduardo Guibelalde. "SSIM Index as a
 * Java plugin for ImageJ" Department of Radiology, Faculty of Medicine.
 * Universidad Complutense. Madrid. SPAIN.
 * http://www.ucm.es/info/fismed/SSIM_family/SSIM.htm
 * 
 * 
 * THIS IS AN AN IMPLEMENTATION AS A PLUGIN FOR IMAGEJ DEVELOPED IN JAVA. IT
 * CALCULATES THE SSIM INDEX (SINGLE-SCALE) AND IT USES THE ZHOU WANG'S
 * ALGORITHM DEVELOPED IN MATLAB. IT WORKS WITH 8, 16 AND 32 BITS GRAY LEVELS.
 ...
 * height; 2) Average local F by F pixels and then downsample the image by a
 * factor of F; and 3) apply the ssim_index.m program. For example, for an 512
 * by 512 image, F = max(1, round(512/256)) = 2, so the image should be average
 * within a 2 by 2 window and downsampled by a factor of 2 before applying
 * ssim_index.m "
 * 
 * THIS VERSION INTRODUCES A CONTROL TO GET THE ACTUAL VIEWING DISTANCE. YOU CAN
 * CHANGE THE PROPOSED VALUE. IT IS ONLY A DOWNSAMPLING FACTOR.
 * 
 */

public class StructuralSimilarity extends ImageSSIMAlgorithm {
	...
	protected void showDialog() {
		while (!out && mode != mode.DEFAULT) {
			out = true;
			gd = new GenericDialog("Filter parameters");
	...
	public void actualAlgorithm() {
		...
		double suma = 0;
		for (pointer = 0; pointer < image_dimension; pointer++) {
			ssim_map[pointer] = (double) ((2 * mu1_mu2[pointer] + C1) * (2 * sigma12[pointer] + C2))
